...
optim_g = optim.Adam(gene.parameters(), lr=lr_G, betas=(0.0,0.9))
optim_d = optim.Adam(disc.parameters(), lr=lr_D, betas=(0.0,0.9))
...
def train():
	...
	def disc_train():
		with autocast():
			...
			real_label_x = real_label[:,0]
			real_label_y = real_label[:,1]

			z, fake_label_x, fake_label_y = gen_input(batch_size)
			fake_data = gene(z).detach()

			disc_real, disc_real_x, disc_real_y = disc(real_data)
			real_loss = disc_real.mean() + aux_f(disc_real_x, real_label_x.view(-1)-1) + aux_f(disc_real_y, real_label_y.view(-1)-1)
			optim_d.zero_grad()
	...
	def gen_train():
		with autocast():
			...
			g_loss = disc_fake.mean() + aux_f(disc_fake_x, fake_label_x.view(-1)-1) + aux_f(disc_fake_y, fake_label_y.view(-1)-1) + l1_loss 
			optim_g.zero_grad()
	...
	for epoch in range(nb_epoch): 
		...
		gloss=g_losses, dloss=d_losses, real_losses=real_losses, fake_losses=fake_losses, w_losses=w_losses, gp_losses=gp_losses, l1_losses=l1_losses), end=' \n')

		hist = np.concatenate((hist,[[epoch, g_losses.avg, d_losses.avg, real_losses.avg, fake_losses.avg, w_losses.avg, gp_losses.avg, l1_losses.avg]])) 
...
torch.save(gene.state_dict(), gene_path)
torch.save(disc.state_dict(), disc_path)
np.savetxt('history.txt', hist[1:], fmt='%.5f')
